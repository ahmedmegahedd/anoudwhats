...
export async function getPipelineBoard(filters: PipelineFilters = {}) {
...
export async function getPipelineForecast(filters: PipelineFilters = {}) {
  const board = await getPipelineBoard(filters);
  const stageResults = STAGES.map((cfg) => {
...
export async function moveDealStage(
  ...
  const { data, error } = await db
    ...
    .single();
  if (error) throw new BadRequestError(error.message);
  if (!data) throw new NotFoundError(`Contact ${contactId} not found`);
...
export function getPipelineStages(): StageConfig[] {
  return STAGES;
}

export function parsePipelineFilters(sp: URLSearchParams): PipelineFilters {
  const filters: PipelineFilters = {};
  if (sp.get('campaign_id')) filters.campaign_id = sp.get('campaign_id')!;
  if (sp.get('agent_id')) filters.assigned_agent_id = sp.get('agent_id')!;
